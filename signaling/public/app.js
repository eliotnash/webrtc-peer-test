...
const text = {
  zh: {
    ...
    remoteMediaStopped:'远端已停止媒体共享', localCandidate:'本地 ICE 候选：{candidate}',
    remoteCandidate:'远端 ICE 候选：{candidate}', selectedPair:'最终 ICE 候选对：本地 {local} ⇄ 远端 {remote}',
    rtc_new:'新建', rtc_connecting:'连接中', rtc_connected:'已连接', rtc_disconnected:'已断开',
    rtc_failed:'失败', rtc_closed:'已关闭'
  },
  en: {
    ...
    remoteMediaStopped:'Remote media sharing stopped', localCandidate:'Local ICE candidate: {candidate}',
    remoteCandidate:'Remote ICE candidate: {candidate}', selectedPair:'Selected ICE candidate pair: local {local} ⇄ remote {remote}',
    rtc_new:'New', rtc_connecting:'Connecting', rtc_connected:'Connected', rtc_disconnected:'Disconnected',
    rtc_failed:'Failed', rtc_closed:'Closed'
  }
}
...
function connect(roomId) {
  ...
  status('signalStatus','connecting'); log(t('connectingLog',{room:roomId}))
  ws.onopen = () => { status('signalStatus','connected'); log(t('signalConnected')); ws.send(JSON.stringify({type:'join',roomId,peerId,clientType:'browser'})); history.replaceState(null,'',`?room=${roomId}`) }
  ws.onmessage = async event => {
    const data=JSON.parse(event.data)
    if(data.type==='error') return log(data.message)
    if(data.type==='peer-left'){remotePeer=null;status('peerStatus','left');log(t('remoteLeft'));closePeer();return}
    if(data.type==='room-state'){
      const other=data.peers.find(p=>p.peerId!==peerId)
      if(!other){status('peerStatus','waiting');log(t('joinedWaiting'));return}
      const changed=remotePeer!==other.peerId;remotePeer=other.peerId;remoteType=other.clientType
      const typeKey=other.clientType==='cli'?'cli':'browser';status('peerStatus',typeKey);log(t('remoteJoined',{type:t(typeKey)}))
      if(changed&&peerId.localeCompare(remotePeer)<0) await makeOffer()
      return
    }
    if(data.type==='signal') await handleSignal(data.payload)
  }
  ws.onclose=()=>{status('signalStatus','disconnected');log(t('signalClosed'))}
  ws.onerror=()=>log(t('signalError'))
}
function ensurePeer(){
  if(pc)return pc
  pc=new RTCPeerConnection(rtcConfig);status('rtcStatus','preparing');log(t('peerCreated'))
  pc.onicecandidate=e=>{if(e.candidate){log(t('localCandidate',{candidate:e.candidate.candidate}));signal({candidate:e.candidate})}}
  pc.onconnectionstatechange=()=>{status('rtcStatus','rtc_'+pc.connectionState);if(pc.connectionState==='connected')inspectRoute()}
  pc.ontrack=e=>{const stream=e.streams[0];$('remoteVideo').srcObject=stream;e.track.onmute=()=>{$('remoteVideo').srcObject=null};e.track.onunmute=()=>{$('remoteVideo').srcObject=stream};e.track.onended=()=>{$('remoteVideo').srcObject=null}}
...
async function handleSignal(payload){
  const peer=ensurePeer()
  if(payload.mediaStopped){$('remoteVideo').srcObject=null;log(t('remoteMediaStopped'));return}
  ...
  }else if(payload.candidate){log(t('remoteCandidate',{candidate:payload.candidate.candidate}));try{await peer.addIceCandidate(payload.candidate)}catch(error){log(error.message)}}
}
...
function stopMedia(notify=true){if(localStream){const tracks=localStream.getTracks();if(pc)pc.getSenders().filter(sender=>tracks.includes(sender.track)).forEach(sender=>sender.replaceTrack(null));tracks.forEach(track=>track.stop());localStream=null;$('localVideo').srcObject=null;if(notify)signal({mediaStopped:true});log(t('mediaStopped'))}}
function closePeer(){if(pc)pc.close();pc=null;channel=null;$('remoteVideo').srcObject=null;status('rtcStatus','notEstablished');status('routeStatus','pending')}
function candidateLabel(candidate){if(!candidate)return 'unknown';return `${candidate.candidateType||'?'} ${candidate.address||candidate.ip||'?'}:${candidate.port||'?'} ${candidate.protocol||'?'}`}
async function inspectRoute(){const stats=await pc.getStats();let pair;stats.forEach(r=>{if(r.type==='candidate-pair'&&r.state==='succeeded'&&r.nominated)pair=r});if(!pair)return;const local=stats.get(pair.localCandidateId),remote=stats.get(pair.remoteCandidateId);status('routeStatus',local?.candidateType==='relay'||remote?.candidateType==='relay'?'relay':'direct');log(t('selectedPair',{local:candidateLabel(local),remote:candidateLabel(remote)}))}
